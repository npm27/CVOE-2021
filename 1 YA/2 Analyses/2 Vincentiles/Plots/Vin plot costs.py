##set up
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

dat = pd.read_csv("Vincentiles Costs 3_2_22.csv")

dat['diff'] = dat['Upper'].sub(dat['Lower'])
dat['diff2'] = dat['diff'].div(2)

##set up the initial plot
fig = plt.figure()
fig.set_size_inches(12,16)


##Make the subplots
ax1 = fig.add_subplot(2, 1, 1)
ax2 = fig.add_subplot(2, 1, 2)

##might need to subset on trial type
alt_local = dat[dat['Trial Type'] == "Alt_Local"]
alt_global = dat[dat['Trial Type'] == "Alt_Global"]
rand_local = dat[dat['Trial Type'] == "Rand_Local"]
rand_global = dat[dat['Trial Type'] == "Rand_Global"]

##alt local
x2 = alt_local.bin.values
y2 = alt_local.Average.values

ax1.plot(x2, y2, marker = 's', color = 'dimgray', label='Alternating Runs')
ax1.errorbar(x2, y2, yerr = (alt_local['diff2']), fmt='none', c= 'dimgray', capsize=5)

##rand kicak
x3 = rand_local.bin.values
y3 = rand_local.Average.values

ax1.plot(x3, y3, marker = 's', color = 'darkgray', label='Random')
ax1.errorbar(x3, y3, yerr = (rand_local['diff2']), fmt='none', c= 'darkgray', capsize=5)

##Make the plot spiffy
ax1.set_title('Local Switch Costs', fontsize = 20, fontweight = 'bold')
ax1.set_ylabel('Mean RT (ms)', fontsize = 18)
ax1.set_xlabel('Vincentile Bin', fontsize = 18)
ax1.tick_params(axis='x', labelsize = 16)
ax1.tick_params(axis='y', labelsize = 16)

ax1.legend()

##alt global
x4 = alt_global.bin.values
y4 = alt_global.Average.values

ax2.plot(x4, y4,  '--', marker = 's', color = 'dimgray', label='Alternating Runs')
ax2.errorbar(x4, y4, yerr = (alt_global['diff2']), fmt='none', c= 'dimgray', capsize=5)

##rand switch
x5 = rand_global.bin.values
y5 = rand_global.Average.values

ax2.plot(x5, y5, '--', marker = 's', color = 'darkgray', label='Random')
ax2.errorbar(x5, y5, yerr = (rand_global['diff2']), fmt='none', c= 'darkgray', capsize=5)

##Make the plot spiffy
ax2.set_title('Global Switch Costs', fontsize = 20, fontweight = 'bold')
ax2.set_ylabel('Mean RT (ms)', fontsize = 18)
ax2.set_xlabel('Vincentile Bin', fontsize = 18)
ax2.tick_params(axis='x', labelsize = 16)
ax2.tick_params(axis='y', labelsize = 16)

ax2.legend()

fig
#fig.savefig('YA_Vincentiles Costs.png')
